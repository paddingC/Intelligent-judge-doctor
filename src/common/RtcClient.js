...
export default class RtcClient {
    ...
    async join() {
        ...
        try {
            ...
            this.isJoined_ = true
            store.commit('user/SET_AGAINJOIN', true)
            ...
            store.dispatch('app/savelog', {userid: this.userId_, msg: '进房错误'}).then(res=>{
                console.log(res);
            }).catch(err=>{
                console.log(err);
            })
            alert('由于网络异常，加入视频房间超时，正在再次尝试，请点击确认按钮')
    ...
    async againJoin () {
        if (this.networkCount >= 3) {
            ...
            alert('由于网络异常，加入视频房间超时，请先确认网络状况后点击确认按钮，手动进入')
            return false
        }
        try {
            await this.join()
            store.commit('user/SET_AGAINJOIN', false)
            clearInterval(this.networkTimer)
            return false
        } catch (error) {
            console.log(error.log);
        }
        console.log('测试次数' + this.networkCount);
    }
    async leave() {
        if (!this.isJoined_) {
    ...
    handleEvents() {
        ...
        this.client_.on('stream-subscribed', evt => {
            ...
            console.log('远端流订阅成功');
            //if (sessionStorage.getItem('hasCreate') !== 'true') {
                timer = setInterval(()=>{
                    if (count >= 5){
                        clearInterval(timer)
                        return false
                    ...
                    axios({
                        ...
                        console.log(res.data.data.streamState);
