    ...
    <div class="chronicDisease">
        <div class="div1">
            <div class="video" id="remote_stream">
                <!-- 远端-->
                <el-button type="danger" @click="againjoinEvent" v-if="!againjoin">再次进入</el-button>
                <section class="video_local" id="local_stream"></section>
                <!--<img class="videoBG" src=""/>-->
            </div>
            <div class="patient-info">
                <h3 class="border-left"> 患者信息</h3>
                <p><span>姓名</span>: <span>{{doorInfo.inquiryRequestInfo.patient}}</span></p>
                <p><span>性别</span>: <span>{{doorInfo.inquiryRequestInfo.sex}}</span></p>
                <p><span>年龄</span>: <span>{{doorInfo.inquiryRequestInfo.age}}</span></p>
                <p><span>联系电话</span>: <span>{{doorInfo.inquiryRequestInfo.telphone}}</span></p>
                <p><span>身份证号</span>: <span>{{doorInfo.inquiryRequestInfo.idcardno}}</span></p>
                <p><span>门店</span>: <span>{{doorInfo.inquiryRequestInfo.deptname}}</span></p>
            </div>
        </div>
        <div class="div2 patient-info">
            ...
            <div style="margin-top: 20px; width: 100%">
                <div style="width: 30%;display: inline-block">
                    ...
                    <el-select v-model="diagnosisArr"
                               @change="diagnosisarrchange"
                               placeholder="请选择">
                        <el-option
            ...
            <div class="radioTable">
                <vxe-table ref="formList" min-height="320px" max-height="400px"
                          border :header-cell-style="rowClass"
                          :data.sync="tableData" highlight-current-row @current-change="curRow"
                    ...
                    <vxe-table-column field="goodsqty" title="数量" sortable width="180">
                        <template scope="scope">
                            <el-input-number v-if="scope.row.specialdrug == '1'" v-model="scope.row.goodsqty" :min="1" :max="2" @focus="getFocus" style="width: 140px"></el-input-number>
                            <el-input-number v-else v-model="scope.row.goodsqty" :min="1" :max="100" @focus="getFocus" style="width: 140px"></el-input-number>
                        </template>
                    </vxe-table-column>
                    ...
                    <vxe-table-column title="操作" fixed="right" width="70">
                        <template slot-scope="scope">
                            <el-button type="danger" :disabled="isRemove" size="mini" @click="removeEvent(scope.row)">删除</el-button>
                        </template>
                    </vxe-table-column>
                </vxe-table>
            </div>
        ...
        <el-dialog title="药品库明细" :visible.sync="drugVisible">
            <div style="margin-top: 10px; margin-bottom: 10px">
                <el-input v-model="filterName" :autofocus="true" @input="filterVal" style="width: 268px" @keyup.enter.native="diaSearch($event)" type="search" placeholder="请输入您要搜索的内容"></el-input>
            </div>
            <div v-loading="loading">
                <vxe-table ref="formDrug" id="getTable"
                           border :header-cell-style="rowClass"
                           :data.sync="drugList"
                           highlight-current-row
        ...
        <!--录入患者省份证号-->
        <el-dialog title="录入患者身份证号" :visible.sync="iscardVisible" width="620px"
                   :close-on-click-modal="false"
                   :showClose="false">
            <el-form :model="form" ref="form" :rules="rules" label-width="120px" class="demo-ruleForm">
                <el-form-item required prop="idcardno">
                    <el-input style="width: 286px;" type="idcardno" v-model="form.idcardno" autocomplete="off"></el-input>
                </el-form-item>
            </el-form>
...
export default {
    ...
    mounted () {
        ...
        this.getDiseases()
        this.$nextTick(() => {
            this.$store.dispatch('app/savelog', {userid: this.userInfo.userid, msg: '进房之前'}).then(res=>{
                console.log(res);
            }).catch(err=>{
                console.log(err);
            })
    ...
    methods: {
        ...
        againjoinEvent () {
            this.join()
            this.$store.commit('user/SET_AGAINJOIN', false)
        },
        enterRoom (msg) {
            const that = this
        ...
        filterVal(e){
            console.log(e);
            this.$store.dispatch('goods/goodsFind', {
        ...
        getDiseases(){
            this.$store.dispatch('user/diseasesSelect', {
                companyid: this.userInfo.companyid,
                key: ''
            }).then(res => {
                let resVal = res
        ...
        historyRecipe () {
            ...
            this.$store.dispatch('user/sxfinddoc', {
                companyid: this.userInfo.companyid,
        ...
        finddtlEvent ({row}) {
            ...
            this.$store.dispatch('user/finddtl', {
        ...
        getEnd (getTableData) {
            ...
            this.$store.dispatch('user/callEnd', newObject).then(res=>{
                this.rtc.leave()
        ...
        saveDate(){
            ...
            getTableData.fullData.forEach((item, idx) => {
                if (item.yfyl == null || item.yfyl == '' || item.goodsname == null || item.goodsname == '' || item.goodsqty == null || item.goodsqty == '') {
                    this.$message.warning('请输入药品名、数量、用法用量后保存！')
                    isNaN = true
                    return false
                } else {
                    if (item.yfyl.includes('M') === true  && item.yfyl.includes('N') === true) {
                        if (item.jl == null || item.jl == '' || item.pc == null || item.pc == '') {
        ...
        sendMsg () {
            ...
            this.$socket.emit('sendMsgToUser', JSON.stringify({
                type: '1',
                companyid: this.userInfo.companyid,
                userid: this.userInfo.userid,
